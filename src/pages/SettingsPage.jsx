...
export function SettingsPage() {
  ...
  useEffect(() => {
    ...
    endpoints.account.summary().then((result) => {
      ...
      setForm({ username: value.username || user?.username || user?.name || "", defaultPerformance: result?.settings?.performance || value.defaultPerformance || "medium", terminal: result?.settings?.preferredTerminal || value.terminal || "gmgn", askWhenUncertain: true });
  ...
  const save = async () => {
    ...
    try { await endpoints.account.update({ ...form, askWhenUncertain: true }); await refresh(); setSuccess("Settings saved."); }
  ...
  return <div className="standard-page settings-page"><header className="page-title"><div><p>ACCOUNT</p><h1>Settings</h1><span>{loading ? "Loading account details..." : "Control your identity, AI defaults, and connected services."}</span></div><Button loading={saving} onClick={save}>Save changes</Button></header>{error ? <Notice onClose={() => setError("")}>{error}</Notice> : null}{success ? <Notice type="success" onClose={() => setSuccess("")}>{success}</Notice> : null}
    <div className="settings-layout"><div className="settings-content">
      ...
      <section className="settings-card"><div className="settings-heading"><span><Bot/></span><div><h2>AI performance</h2><p>Choose the default balance between capability and credit usage.</p></div></div><div className="mode-list">{modes.map((mode) => <button key={mode.id} className={form.defaultPerformance === mode.id ? "active" : ""} onClick={() => setForm({ ...form, defaultPerformance: mode.id })}><span><Zap/></span><div><strong>{mode.name}</strong><p>{mode.text}</p></div><i>{form.defaultPerformance === mode.id ? <Check/> : null}</i></button>)}</div><label className="toggle-row"><div><strong>Ask when uncertain</strong><small>PAN always pauses for missing or ambiguous details instead of assuming.</small></div><input type="checkbox" checked readOnly disabled/><span/></label></section>
