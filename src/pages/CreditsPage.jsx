...
import { ArrowDownToLine, ArrowUpRight, ChevronDown, Clipboard, Coins, Copy, Flame, Gift, History, RefreshCw, ShieldCheck, Wallet } from "lucide-react";
...
export function CreditsPage() {
  ...
  const load = useCallback(async (quiet = false) => {
    ...
    const results = await Promise.allSettled([endpoints.credits.summary(), endpoints.credits.history()]);
    let nextData = null;
    let nextWallets = null;
    if (results[0].status === "fulfilled") { nextData = results[0].value; setData(nextData); }
