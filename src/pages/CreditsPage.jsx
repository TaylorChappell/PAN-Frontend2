...
export function CreditsPage() {
  ...
  const operational = wallets.find((item) => ["operational", "spending", "launch"].includes(item.type)) || data?.operationalWallet || wallets[1] || {};
  const payment = wallets.find((item) => ["payment", "credits", "conversion"].includes(item.type)) || data?.paymentWallet || wallets[0] || {};
  ...
  const createPurchase = async () => { setWorking(true); setError(""); try { const result = await endpoints.credits.purchase({ asset: method === "pan" ? "PAN" : "ETH", amount: Number(amount), expectedCredits: method === "pan" ? Number(amount) : undefined }); setIntent(result?.intent || result); } catch (e) { setError(e.message); } finally { setWorking(false); } };
